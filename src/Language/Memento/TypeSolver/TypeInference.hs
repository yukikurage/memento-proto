...
import Language.Memento.TypeSolver.SolverPipeline (createDefaultPipeline, executePipeline, SolverState(..))
...
inferTypes ast = do
  ...
  finalTypeEnv <- checkProgramBodies ast programTypeEnv (vaVariances varianceAnalysis)
    ...
    SDefinition.ValDef varAst typeParams typeAst exprAst -> do
      ...
      case Map.lookup name currentEnv of
        Just declaredScheme -> do
          ...
          constraintEnv <- buildConstraintEnv currentEnv variances
...
buildConstraintEnv typeEnv variances = do
