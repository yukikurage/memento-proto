...
import Language.Memento.TypeSolver.Solver (branchConstraints, calculatePropagationAll, checkContradictions, decomposeConstraintsAll, normalizeConstraint, substInstancesAsPossible)
...
data SolverState = SolverState
  { ssConstraints :: ConstraintSet,             -- ^ Current constraints
    ssAssumptions :: AssumptionSet,             -- ^ Current assumptions  
    ssGenericBounds :: GenericBoundsMap,        -- ^ Generic bounds map
    ssVariances :: TypeConstructorVariances,    -- ^ Type constructor variances
    ssStageHistory :: [T.Text]                  -- ^ History of stages executed (for debugging)
  }
  deriving (Show, Eq)

-- | Result of executing a solver stage
data SolverResult m
  = SolverContinue SolverState                  -- ^ Continue with updated state
  | SolverBranch [(SolverState, SolverPipeline m)] -- ^ Branch into multiple paths
  | SolverSuccess                               -- ^ Solving completed successfully
  | SolverFailure TypeError                     -- ^ Solving failed with error

-- | A single stage in the solver pipeline
data SolverStage m = SolverStage
  { stageName :: T.Text,                        -- ^ Name for debugging
    stageFunction :: SolverState -> m (SolverResult m) -- ^ Stage implementation
  }
...
data SolverPipeline m = SolverPipeline
  { pipelineStages :: [SolverStage m],          -- ^ Ordered list of stages
    pipelineName :: T.Text                      -- ^ Pipeline name for debugging
  }

instance Show (SolverPipeline m) where
  show pipeline = "SolverPipeline{pipelineName=" ++ T.unpack (pipelineName pipeline) ++ 
                  ", stages=[" ++ show (map stageName (pipelineStages pipeline)) ++ "]}"
...
createDefaultPipeline = SolverPipeline
  { pipelineStages = 
    [ normalizationStage
    , decompositionStage
    , boundsCalculationStage
...
normalizationStage = SolverStage
...
decompositionStage = SolverStage
...
boundsCalculationStage = SolverStage
...
branchingStage = SolverStage
  ...
  where
    ...
    createBranchState originalState (branchAssumptions, branchConstraints) = SolverState
    ...
    createPostBranchPipeline = SolverPipeline
...
contradictionStage = SolverStage
...
createCustomPipeline name stages = SolverPipeline
...
createInitialState variances assumptions constraints = SolverState
